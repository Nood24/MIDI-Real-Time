...
void DanceSet::load_instruments() {
    ...
    Instrument* accordion = new Instrument(this->file_location + "Accordion.csv", this->tempo, this->hardware, 1, 1, 1,this,0,65);
...
void DanceSet::set_notes(std::vector< unsigned char >* message){
    ...
    if (this->previousBass!=this->bassNote || !checkEqual(this->previousChord,this->chordNotes)){
        for (int i=0; i<4; i++){
            instruments[i]->update_note(this->previousBass!=this->bassNote, this->previousChord!=this->chordNotes);
        }
    }
}
